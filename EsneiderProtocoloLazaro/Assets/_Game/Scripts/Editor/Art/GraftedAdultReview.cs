...
{
    public static class GraftedAdultReview
    {
        public const string Asset = "HUM-02_AdultoInjertos";
        public const string Support = "HUM-02_SoporteInclinado";
        public const string Prefab = "Assets/_Game/Prefabs/Environment/HUM-02_AdultoInjertos.prefab";
        public const string Assembly = "Assets/_Game/Prefabs/Environment/HUM-02_SujetoRestringido.prefab";
        ...
        public static string Integrate()
        {
            ...
            var implant = AssetIntegrator.BuildMaterial(Asset + "_Implant", report);
            skin.SetFloat("_BumpScale", .25f); skin.SetFloat("_Smoothness", .6f);
            cloth.SetFloat("_BumpScale", .5f);
            implant.SetFloat("_BumpScale", .45f); implant.SetFloat("_Smoothness", .7f); implant.SetFloat("_Metallic", 1f);
            EditorUtility.SetDirty(skin); EditorUtility.SetDirty(cloth); EditorUtility.SetDirty(implant);
            ...
            try
            {
                ...
                if (renderers.Length != 3) throw new InvalidOperationException("Expected skin, cloth and implant meshes");
                foreach (var r in renderers)
                {
                    r.sharedMaterial = r.name.Contains("Skin") ? skin : r.name.Contains("Cloth") ? cloth : implant;
                ...
                if (report.tris > 21000 || report.submeshes != 3) throw new InvalidOperationException("Subject exceeds budget");
                var clip = AssetDatabase.LoadAllAssetsAtPath(fbx).OfType<AnimationClip>()
                    .Single(c => !c.name.StartsWith("__preview") && c.name.Contains("Restrained_Breathe"));
                if (clip.length < 5.9f || clip.length > 6.1f) throw new InvalidOperationException("Unexpected ambient clip duration");
                const string controllerPath = "Assets/_Game/Art/HUM02_Restrained.controller";
                ...
                var idle = machine.AddState("Restrained breathing"); idle.motion = clip; machine.defaultState = idle;
                ...
                report.clips = 1; report.clipNames.Add(clip.name);
                report.materials.Add(skin.name); report.materials.Add(cloth.name); report.materials.Add(implant.name);
                File.WriteAllText(Path.Combine(FurnitureReview.Evidence(Asset), "unity_import.json"), JsonUtility.ToJson(report, true));
                AssetDatabase.SaveAssets(); return report.tris + " tris, three skinned meshes, six-second restrained loop";
            }
        ...
        public static string Assemble()
        {
            ...
            var root = new GameObject("HUM-02_SujetoRestringido");
            try
            {
                var pivot = new GameObject("ReclinePivot").transform; pivot.SetParent(root.transform, false);
                // M042 is viewed from the east aisle; the subject faces that aisle.
                pivot.localRotation = Quaternion.Euler(12f, -90f, 0);
                var rack = (GameObject)PrefabUtility.InstantiatePrefab(
                    AssetDatabase.LoadAssetAtPath<GameObject>(FurnitureReview.Prefab(Support)), pivot);
                rack.transform.localPosition = new Vector3(0, 0, .16f);
                var human = (GameObject)PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath<GameObject>(Prefab), pivot);
                human.transform.localPosition = new Vector3(0, .08f, 0); human.transform.localRotation = Quaternion.identity;
                var subject = human.GetComponent<Esneider.World.PreservedSubject>(); subject.phaseOffset = .61f;
                PrefabUtility.RecordPrefabInstancePropertyModifications(subject);
                PrefabUtility.SaveAsPrefabAsset(root, Assembly);
                return "Grafted adult on a reclined rack with foot and cervical support";
            }
        ...
        public static string Apply(Transform regionRoot, string region)
        {
            ...
            var cage = regionRoot.GetComponentsInChildren<Transform>(true).Single(t => t.name == "M042_jaula");
            if (cage.Find("M042_GraftedAdult_EX45")) return "";
            var old = cage.Find("Sujeto_placeholder_M042");
            if (!old) throw new InvalidOperationException("M042 subject placeholder missing");
            var wrapper = new GameObject("M042_GraftedAdult_EX45").transform;
            ...
            return "M042 capsule replaced by grafted adult on reclined restraint rack";
        }
