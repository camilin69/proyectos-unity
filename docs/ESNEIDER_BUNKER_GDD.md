...
## Instrucciones iniciales para Claude: uso obligatorio de las skills instaladas

**Claude: antes de planificar o ejecutar este encargo, revisa todas las skills instaladas y utiliza sus instrucciones durante la producción.** Este bloque aparece en el maestro y en el anexo como un mismo contrato; no duplica tareas. La petición vigente del usuario y la precedencia del maestro conservan la autoridad sobre el alcance.

1. **Localiza y lee todas las skills.** La instalación comprobada está en `C:/Users/HP/.claude/skills/<nombre>/SKILL.md`. En otro computador, busca su equivalente en `~/.claude/skills` y en `.claude/skills` del proyecto. Lee todos los `SKILL.md` del inventario siguiente y consulta sus recursos de apoyo al aplicarlos. Comprueba disponibilidad real en destino; si falta alguna, registra nombre, ruta e impacto, continúa lo que no dependa de ella y no afirmes haberla utilizado.
2. **Incluye todas en el plan de producción.** Prepara una matriz con skill, etapa/requisito/asset, condición de activación, entregable y evidencia. Aplica cada skill cuando corresponda y registra las pendientes o no aplicables con motivo concreto. No añadas XR, multijugador, ECS/DOTS, sistemas 2D, logos ni trabajos financieros solo para activar una skill: estos requieren un encargo que los incluya.
3. **Coordina el trabajo de Blender.** Usa `blender-skill-harmonizer` para resolver dependencias y solapamientos y `blender-pro-workflow` para organizar la producción. Encadena modelado, materiales, UV/texturas, iluminación, cámaras, render, animación y exportación según las fichas. Aplica reconstrucción, registro, ajuste y reparación cuando existan referencias pertinentes, y los controles de calidad y refinamiento antes de declarar un asset conforme. Conserva las fuentes, la escala y los criterios de aceptación.
4. **Aplica las skills de Unity por sistema.** Relaciona arquitectura, scripting, escenas/prefabs, ciclo de juego, entrada, física, IA/navegación, animación, audio, cámaras, interfaz, guardado, gráficos, iluminación, rendimiento y pruebas con los contratos del maestro. Comprueba versiones y paquetes reales antes de usar las APIs de una skill; sus ejemplos no cambian automáticamente el perfil técnico del proyecto.
5. **Ejecuta con ambos MCP y verifica.** Las skills guían el trabajo y no sustituyen las herramientas: utiliza activamente los MCP de Blender y Unity conforme a la sección 100 del maestro. Ejecuta `bpy` dentro de Blender mediante su MCP; verifica en destino las rutas de intérpretes y dependencias auxiliares. Fabrica, exporta, integra y prueba archivos reales; conserva fuentes y evidencia de aceptación.
6. **Registra el uso durante la producción.** Mantén la matriz por etapa y registra en la sección 106 del maestro y en las fichas las skills aplicadas, resultados, archivos, capturas, pruebas y pendientes. Leer una skill no demuestra que se haya ejecutado ni que un asset sea conforme. Resuelve decisiones rutinarias dentro de la autonomía del maestro y eleva los conflictos que excedan esa autonomía. Este bloque prepara el futuro encargo de construcción; no certifica una build existente ni ordena construir durante esta actualización documental.

**Inventario verificado para Claude (69 skills):**

- **Blender, referencias y control de calidad:** `animation-quality-gate`, `atlas-uv-fitting`, `blender-animation`, `blender-cameras`, `blender-export`, `blender-lighting`, `blender-materials`, `blender-modeling`, `blender-pro-workflow`, `blender-rendering`, `blender-skill-harmonizer`, `blender-uv-texturing`, `closed-surface-uv-coverage`, `contour-to-mesh`, `fit-repair-optimizer`, `landmark-fit-repair`, `mascot-logo-reconstruction`, `multiview-constraint-solver`, `multiview-fit-loop`, `orbital-hud-motion`, `orthographic-registration`, `quality-refinement-autoloop`, `reference-analysis-validator`, `reference-look-calibration`, `reference-to-3d`, `source-part-segmentation`, `text-to-blender`, `texture-driven-mesh-fitting`, `texture-state-animation`, `wireframe-to-3d`.
- **Unity:** `unity-2d`, `unity-3d-math`, `unity-ai-navigation`, `unity-animation`, `unity-async-patterns`, `unity-audio`, `unity-cinemachine`, `unity-data-driven`, `unity-ecs-dots`, `unity-editor-tools`, `unity-foundations`, `unity-game-architecture`, `unity-game-loop`, `unity-graphics`, `unity-input`, `unity-input-correctness`, `unity-level-design`, `unity-lifecycle`, `unity-lighting-vfx`, `unity-multiplayer`, `unity-npc-behavior`, `unity-packages-services`, `unity-performance`, `unity-physics`, `unity-physics-queries`, `unity-platforms`, `unity-procedural-gen`, `unity-save-system`, `unity-scene-assets`, `unity-scripting`, `unity-state-machines`, `unity-testing`, `unity-ui`, `unity-ui-patterns`, `unity-xr`.
- **Negocio, economía y finanzas:** `economista`, `emprendedor`, `financiero-senior`, `inversionista`. Aplícalas si se solicita viabilidad, presupuesto, financiación o análisis comercial. La economía interna del gameplay sigue los contratos del maestro.
