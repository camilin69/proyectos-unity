...
|---|---|---|
| EditMode (`Esneider.Tests.EditMode`): contratos de núcleo, guardado robusto (truncado, checksum, versión futura, temporal huérfano, rotación), economía 80.1, contacto del pie y del agarre sobre el FBX importado | 25/25 | `evidencia/tests_EditMode.md` |
| PlayMode (`Esneider.Tests.PlayMode`): sandbox QA-01…08, IA del Vigía, persistencia QA-09…11, streaming, narrativa EVT-01/C1, sectores (patrullas/mobiliario/EVT-16/18/C2), jefe, menús, recorrido completo O01→O11, física de mundo PIL-08 | 35/35 | `evidencia/tests_PlayMode.md` |
...
| FIN-08 animación | **Pasa en contactos medidos; sin captura de movimiento** | `SourceArt/Scripts/measure_contacts.py` mide cada clip frame a frame contra umbrales escritos y emite `_evidence/EX-08/contacts_*.json`. Los tres bots pasan ahora los cinco gates de contacto: despeje del pie en vuelo 80 / 108 / 174 mm (mínimo 30), dispersión de velocidad del pie apoyado 0 % (máximo 35), rebote vertical 0.0 mm, fracción de apoyo 1.00 y velocidad del clip a menos del 2 % de la del dato. Los ciclos se generan por cinemática inversa de dos eslabones (`esn_lib.walk_cycle`), no por ángulos escritos a mano. `WalkContactTests` repite la medición sobre el FBX ya importado en Unity y obtiene los mismos números. El agarre del arma también está medido y en tolerancia: el mango cae a 10-20 mm de la palma, las yemas entre 0 y 16 mm de su superficie y el pulgar se opone con 3-6 mm de compresión, sin atravesarlo (`EX-09_grip_contact_sheet.png`). Para conseguirlo hubo que dar tres falanges a cada dedo: con un solo hueso el dedo giraba rígido y la yema pasaba de largo. Corregido antes: los clips de muerte enterraban el cuerpo (Vigía 361 mm, Custodio 600, Archivista 876) y el arma colgaba de la cámara en vez de la mano. Sigue sin captura de movimiento real. Evidencia visual: `_evidence/EX-08/EX-09_walk_contact_sheet.png`. |
...
Un tercer defecto se detectó y corrigió sin llegar a entregarse: al medir las curvas de los clips **ya importados en Unity** salieron 36 claves por curva y cero curvas con cambio de valor, es decir todos los clips como pose estática. El exportador FBX evalúa las tiras NLA a través del stack, y el medidor de contactos lo dejaba apagado. Corregido en `export_fbx`, reexportados los 16 assets y cubierto con `WalkContactTests` para que no pueda repetirse en silencio.
