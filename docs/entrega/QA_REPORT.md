...
|---|---|---|
| EditMode (`Esneider.Tests.EditMode`): contratos de núcleo, guardado robusto (truncado, checksum, versión futura, temporal huérfano, rotación), economía 80.1, contacto del pie sobre el FBX importado | 24/24 | `evidencia/tests_EditMode.md` |
...
| FIN-08 animación | **Pasa en contactos medidos; sin captura de movimiento** | `SourceArt/Scripts/measure_contacts.py` mide cada clip frame a frame contra umbrales escritos y emite `_evidence/EX-08/contacts_*.json`. Los tres bots pasan ahora los cinco gates de contacto: despeje del pie en vuelo 80 / 108 / 174 mm (mínimo 30), dispersión de velocidad del pie apoyado 0 % (máximo 35), rebote vertical 0.0 mm, fracción de apoyo 1.00 y velocidad del clip a menos del 2 % de la del dato. Los ciclos se generan por cinemática inversa de dos eslabones (`esn_lib.walk_cycle`), no por ángulos escritos a mano. `WalkContactTests` repite la medición sobre el FBX ya importado en Unity y obtiene los mismos números. Corregido antes: los clips de muerte enterraban el cuerpo (Vigía 361 mm, Custodio 600, Archivista 876) y el arma colgaba de la cámara en vez de la mano. Sigue sin captura de movimiento real. Evidencia visual: `_evidence/EX-08/EX-09_walk_contact_sheet.png`. |
